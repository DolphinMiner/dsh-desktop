...
export async function resolveWorkspaceTarget(
  ...
  if (!targetStats.isFile()) {
    throw new WorkspacePathError('BAD_MESSAGE', 'Only regular workspace files can be opened.')
  }
  if ((targetStats.mode & 0o111) !== 0 || BLOCKED_OPEN_EXTENSIONS.has(extname(canonicalTarget).toLowerCase())) {
