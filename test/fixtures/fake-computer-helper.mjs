...
if (process.env.DSH_COMPUTER_FIXTURE_MODE === 'hang') {
  ...
  response({ supported: true, screenRecording: 'granted', accessibility: 'denied', canObserve: true })
} else if (request.method === 'listTargets') {
  response({
    permissions: { supported: true, screenRecording: 'granted', accessibility: 'denied', canObserve: true },
    targets: [{
      id: 'application:42',
      kind: 'application',
      name: 'Editor',
